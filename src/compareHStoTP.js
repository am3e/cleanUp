...
const datadiscoveryFile = `${today.toISODate()}-teampList.csv`;
...
const processFiles = async () => {
    ...
    const ddToHubspotByEmail = (ddContact) => {
        for( let i = 0; i < ddEmailFields.length; i++) {
            const field = ddEmailFields[i];
            if (ddContact[field]) {
                const emails = ddContact[field].split(',');
                const email = emails.find(email => hubspotByEmail[email]);
                return hubspotByEmail[email];
            }
        }
    }
    const newProspectCheck = datadiscovery.filter( ddContact => {
        //return false if theres a duplicate in hubspot
        const contactsMatchingEmail = ddToHubspotByEmail(ddContact);
        if (contactsMatchingEmail) {
            const hubspotContact = contactsMatchingEmail[0];
    ...
    createCsvWriter({
        path: `${CSV_DIR}${today.toISODate()}-teamPinHS.csv`,
        header: headers,
